...
interface Template {
    id: number; name: string; description: string | null; subject: string | null;
    body: string | null; category_id: number | null; priority: string | null;
    tag_ids: number[] | null; is_active: boolean;
}
interface Props { template: Template; categories: Category[]; tags: Tag[]; }
...
export default function TemplateEdit({ template, categories, tags }: Props) {
    const form = useForm({
        name:        template.name,
        description: template.description ?? '',
        subject:     template.subject ?? '',
        body:        template.body ?? '',
        category_id: template.category_id?.toString() ?? '' as string | number,
        priority:    template.priority ?? '' as string,
        tag_ids:     template.tag_ids ?? [] as number[],
        ...
        <AppLayout>
            ...
            <div className="p-6 max-w-3xl mx-auto space-y-6">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-semibold text-[--color-text]">Edit Template</h1>
                </div>
                ...
                <form onSubmit={submit} className="space-y-5">
                    <div className="bg-white rounded-xl border border-[--color-border] p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input value={form.data.description} onChange={e => form.setData('description', e.target.value)} className={SELECT_CLS} />
                        </div>
                    ...
                    <div className="bg-white rounded-xl border border-[--color-border] p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input value={form.data.subject} onChange={e => form.setData('subject', e.target.value)} className={SELECT_CLS} />
                        </div>
                        <div>
